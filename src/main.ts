...
const fetchAllData = async (url: string) => {
    ...
    const pokemonList: IPokemon[] = await Promise.all(
        dataResults.map(async (pokemon: Result) => await fetchPokemonData(pokemon.name))
    );
...
const renderPokemonList = (pokemonList: IPokemon[]) => {
    ...
    pokemonList.forEach((pokemon: IPokemon) => {
        ...
        <div class="pokemon__box">
                <img class="pokemon__img" src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/dream-world/${
                    pokemon.id
            ...
            <span class="pokemon__type ${pokemon.types[1]?.type.name || ""}">${
...
const fetchAbility = async (name: string): Promise<string> => {
    ...
    const effectEntry = data.effect_entries.find((entry: any) => entry.language.name === "de");
...
const fetchTypeData = async (typeLink: string) => {
    ...
    const pokemonTypeList = await Promise.all(
        data.pokemon
            .slice(0, 20)
            .map(async (item: Pokemon) => await fetchPokemonData(item.pokemon.name))
    );
